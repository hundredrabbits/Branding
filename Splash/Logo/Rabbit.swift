...
class Rabbit : SKNode
{
	...
	func sneak()
	{
		...
		let action_move = SKAction.move(to: moves.last!, duration: 0.45 * animationRatio)
	...
	func appear(_ rank:Int)
	{
		...
		let action_wait = SKAction.wait(forDuration: 0.03 * Double(rank) * animationRatio)
		let action_scale1 = SKAction.scale(to: 1, duration: 0.5 * animationRatio)
